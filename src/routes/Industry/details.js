...
const IndustryDetails = ({ industry, dispatch, location, loading }) => {
  const { dslist, dspagefo, dstype, dsname } = industry;
  // 设置图片列表格式
  const columnp = [
    {
      title: 'ID',
      ...
      <Tabs defaultActiveKey="1">
        <TabPane tab={typeName} key="1">
          <Table
            ...
            columns={column}
            dataSource={dslist}
            loading={loading.effects['industry/detailCommonet']}
...
export default connect(({ industry, loading }) => ({ industry, loading }))(IndustryDetails);
